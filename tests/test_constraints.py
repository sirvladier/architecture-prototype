...
def test_excluded_is_not_ranked_and_reason_is_real():
    ...
    assert result.excluded_alternatives == [{"architecture": "A1", "name": model.architectures[0]["name"], "status": "excluded", "violated_constraints": [{"constraint_id": "C1", "name": model.constraints[0]["name"], "property": "independent_deployment", "actual": False, "operator": "==", "expected": True, "reason": "condition_failed"}]}]
